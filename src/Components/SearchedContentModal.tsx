...
function SearchedCotentModal({ clickedContent, content, keyName, scrollY }: IMovieModal){
    if(content === "person")return null;
  ...
  const onOverlayClick = () => history.push(getRoot["search"]+"?keyword="+keyName);
  ...
  const videoContent = videoData?.results.filter(
      (video) => (video.type === "Teaser" || video.type === "Trailer"|| video.type ==="Featurette" ) 
...
if(!videoLoading && videoContent.length >0){
  ...
  return (<>
       <AnimatePresence>
              ...
              >
                  {clickedContent && !videoLoading && !isLoading && videoContent.length>0 ?
                  (<>
...
export default SearchedCotentModal;